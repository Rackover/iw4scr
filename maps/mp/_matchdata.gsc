...
init()
{
	if ( !isDefined( game["gamestarted"] ) )
	{
		...
		//setMatchData( "dateTime", 0, "index", 1, "woof", 2, getSystemTime() );
	...
	level.MaxLives = 0; // must match MaxKills in matchdata definition
...
logKillstreakEvent( event, position )
{
	...
	eventId = level.CurKillstreak;
	level.CurKillstreak = level.CurKillstreak + 1;
...
//	setMatchData( "eventCount", eventId+1 );
	...
	eventId = level.CurEvent;
	level.CurEvent = level.CurEvent + 1;
	setMatchData( "eventCount", eventId+1 );
